 ...
 * 
 * @deprecated This file is a SHIM. The canonical trust engine is at:
 *   lib/trust/index.ts → lib/trust/trust-score-v3.ts
 * 
 * The V3 trust engine is the STRONGEST version:
 * - 7 weighted components (not just 4)
 * - Leaderboard score (0-1000) with anti-gaming caps
 * - Inactivity decay
 * - Sybil risk detection
 * - Per-country normalization
 * - Persistence helpers for Supabase
 * 
 * This shim re-exports V3 types/functions + original types for backwards compat.
 * New code should import from '@/lib/trust'.
 */

// Re-export the canonical V3 trust engine
export {
    type TrustTier,
    type TrustInput,
    type TrustResult,
...
// Re-export verification tiers for backwards compat
export {
...
// ── Legacy types (kept for backwards compat, map to V3 types) ──
...
export interface TrustInputs {
    ...
    dispute_rate: number;
    incident_free_streak: number;
    total_completions: number;
    corridor_completions: number;
...
/**
 * @deprecated Use computeTrustScore() from lib/trust instead.
 * Legacy function that calculates trust score from old-format inputs.
 */
export function calculateTrustScore(inputs: TrustInputs): TrustScore {
    ...
    if (inputs.insurance_verified) {
        ...
        badges.push({ id: 'insurance', label: 'Insurance Verified', icon: '🛡️', color: daysLeft !== undefined && daysLeft < 30 ? '#F59E0B' : '#10B981', verified: true, expires_at: inputs.insurance_expires_at, days_until_expiry: daysLeft });
    }
    ...
    return { overall, verification: Math.round(verification), reliability: Math.round(reliability), experience: Math.round(experience), freshness: Math.round(freshness), tier, badges, warnings };
}

/**
 * @deprecated Use computeTrustScore() from lib/trust with load-fit engine instead.
 */
