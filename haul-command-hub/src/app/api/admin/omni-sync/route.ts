...
import { POOL_ROLES } from '@/lib/positions-global';
...
export async function POST(req: Request) {
  try {
    ...
    const regPayload = Object.values(regulations).map(r => ({
      country_code: r.code,
      name: r.name,
      authority_name: r.authority.name,
      authority_url: r.authority.url || null,
      max_metric_width: r.legalLimits.width,
      max_metric_height: r.legalLimits.height,
      max_metric_weight: r.legalLimits.weight,
      single_escort_threshold: r.escortRules.singleEscort,
      police_escort_threshold: r.escortRules.policeEscort,
      regulatory_citation: r.legalCitation || null
    }));

    // 3. Process 73 Global Industry Positions
    const posPayload = POOL_ROLES.map(p => ({
      term_id: p.id,
      title: p.title,
      category: p.category,
      countries: p.countries || [],
      is_autonomous: p.category.includes('Autonomous') || false
    }));
