...
serve(async (req) => {
  ...
  const importRecord = await resolveImport({ ...body, source_format: body.source_format ?? "api" });
  ...
  try {
    ...
    const rows = body.rows ?? coerceJsonRows(JSON.parse(await fetchText(sourceUrl)));
    ...
    await updateImport(importRecord.id as string | null, {
      ...
      result_summary: { parser: "authority-api-parser", observations: observations.length, stored },
    });

    return json(200, { ok: true, parser: "authority-api-parser", observations: observations.length, stored });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await updateImport(importRecord.id as string | null, { status: "failed", last_error: message });
    return json(500, { ok: false, parser: "authority-api-parser", error: message });
  }
});
