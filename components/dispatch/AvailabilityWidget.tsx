...
export default function AvailabilityWidget({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={className} style={{background: cfg.bg,border: `1px solid ${cfg.color}22`,borderRadius: 14,padding: '14px 16px',transition: 'all 0.3s'}}>
      {/* Current Status */}
        ...
        {(Object.keys(STATUS_CONFIG) as AvailabilityStatus[]).map(s => {
          ...
          return (
            <button
              ...
              style={{flex: 1,padding: '9px 0',borderRadius: 8,background: isActive ? sc.color : 'rgba(255,255,255,0.04)',border: `1px solid ${isActive ? sc.color : 'rgba(255,255,255,0.08)'}`,color: isActive ? '#fff' : '#94a3b8',fontSize: 11,fontWeight: 700,cursor: updating || isActive ? 'default' : 'pointer',opacity: updating ? 0.5 : 1,transition: 'all 0.2s'}}
            >
              {sc.icon} {sc.label}
            </button>
          );
