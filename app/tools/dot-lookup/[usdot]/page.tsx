...
import { ShieldCheck, Crosshair, Phone, Building, AlertTriangle, Info, ShieldAlert, Award, FileText, Download } from "lucide-react";
import Link from "next/link";
import { Button } from "@/components/ui/button";

// A mock struct to simulate fetching an FMCSA Company dataset.
// In actual production, this would hit the Haul Command API which wraps SAFER/FMCSA.
...
function getMockCompanyMetrics(usdot: string): CompanyData | null {
  // Simulating a DB lookup or API call
  // Fake standard fallback data
  return {
    usdot,
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-b from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-6 relative overflow-hidden">
               ...
               <Button className="w-full bg-slate-100 hover:bg-white text-slate-900 font-bold relative z-10" asChild>
                 <Link href={`/claim/usdot/${data.usdot}`}>Claim Profile — It's Free</Link>
               </Button>
            </div>
...
// Just an internal CheckIcon mapping since I didn't import CheckCircle2 above
