...
function createSlug(baseStr, stateStr, phoneStr) {
    let safeStr = (baseStr || 'truck_stop').toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').slice(0,30);
    let safeState = stateStr.toLowerCase().replace(/[^a-z]/g, '').slice(0,2);
    return `hc-truck_stop-${safeStr}-${safeState}-${phoneStr.slice(0,5)}-claim`;
}

async function run() {
    console.log("[🚀] Launching Playwright Armored Scraper for Pilot Car Truck Stops...");
    ...
    try {
        const url = `https://uspilotcars.com/truck_stop.html`;
        ...
        const pageData = await page.evaluate(() => {
            ...
            while ((node = walker.nextNode())) {
                ...
                // We keep slightly shorter blocks because truck_stops might just be "ATREA INN"
        ...
        for (let j = 0; j < pageData.length; j++) {
            ...
            if (phoneMatch) {
                ...
                let name = "Pilot Car Friendly Truck Stop";
                let city = "Regional";
                    ...
                    if (j - k >= 0) {
                        ...
                        if (!prev.match(/\b\d{3}-\d{4}\b/)) {
                            ...
                            if (prev.includes('INN') || prev.includes('MOTEL') || prev.includes('HOTEL') || prev.includes('SUITES') || prev.includes('WYNDHAM') || prev.includes('LODGE')) {
                                ...
                                if (name === "Pilot Car Friendly Truck Stop") name = prev;
                                else city = prev;
                            }
                            // Detect amenities
    ...
    console.log(`\n[🚀] Truck Stop Extraction Complete! Pulled ${allExtracted.length} potential truck_stop spots.`);
    ...
    if (finalBatch.length > 0) {
        ...
        if (error) {
            ...
            console.log(`[+] ✅ TRUCK STOP DIRECTORY SEEDED: ${finalBatch.length} Heavy-Haul Friendly Truck Stops deployed. Claim Funnel Active.`);
        }
    }
}
