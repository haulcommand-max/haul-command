...
export function IntentToActionBar({
    ...
    return (
        <div style={{display: 'grid',gridTemplateColumns: `repeat(${Math.min(items.length, 5)}, 1fr)`,gap: 8}}>
            {items.map(a => (
                <Link aria-label="Navigation Link"
                    ...
                    style={{padding: '14px 10px',borderRadius: 14,textAlign: 'center',background: a.primary ? `${a.color}10` : 'rgba(255,255,255,0.02)',border: `1px solid ${a.primary ? `${a.color}25` : 'rgba(255,255,255,0.06)'}`,textDecoration: 'none',transition: 'all 0.1s ease'}}
                >
                    <div style={{fontSize: 20,marginBottom: 4 }}>{a.icon}</div>
                    <div style={{fontSize: 10,fontWeight: 800,color: a.primary ? a.color : '#fff'}}>
                        {a.label}
                    </div>
                </Link>
            ))}
...
export function RelatedLinksModule({
    ...
    return (
        <div style={{marginTop: 24 }}>
            <div style={{fontSize: 10,fontWeight: 900,color: '#888',textTransform: 'uppercase',letterSpacing: '0.1em',marginBottom: 10}}>
                {title}
            </div>
            <div style={{display: 'flex',flexDirection: 'column',gap: 6 }}>
                {links.map(link => (
                    <Link aria-label="Navigation Link"
                        key={link.href}
                        href={link.href}
                        style={{display: 'flex',justifyContent: 'space-between',alignItems: 'center',padding: '10px 14px',borderRadius: 10,background: 'rgba(255,255,255,0.02)',border: '1px solid rgba(255,255,255,0.06)',textDecoration: 'none'}}
                    >
                        <span style={{fontSize: 13,fontWeight: 600,color: '#fff' }}>{link.label}</span>
                        {link.badge && (
                            <span style={{fontSize: 9,fontWeight: 800,padding: '2px 8px',borderRadius: 6,background: 'rgba(241,169,27,0.08)',color: '#F1A91B'}}>
                                {link.badge}
                            </span>
                        )}
                    </Link>
...
export function ReadinessState({
    ...
    return (
        <div style={{display: 'flex',alignItems: 'center',gap: 10,padding: '10px 14px',borderRadius: 12,background: `${config.color}06`,border: `1px solid ${config.color}12`}}>
            <div style={{width: 8,height: 8,borderRadius: '50%',background: config.color,boxShadow: `0 0 8px ${config.color}40`,flexShrink: 0}} />
            <div style={{flex: 1 }}>
            ...
            <Link aria-label="Navigation Link" href={config.cta.href} style={{padding: '6px 12px',borderRadius: 8,fontSize: 10,fontWeight: 800,background: `${config.color}12`,border: `1px solid ${config.color}20`,color: config.color,textDecoration: 'none',whiteSpace: 'nowrap'}}>
                {config.cta.label}
            </Link>
        </div>
    );
    ...
    return (
        <div style={{padding: '48px 16px 24px' }}>
            ...
            {breadcrumbs && breadcrumbs.length > 0 && (
                <div style={{display: 'flex',gap: 6,alignItems: 'center',marginBottom: 16,fontSize: 11,color: '#888'}}>
            ...
            {/* Market mode badge */}
            {marketMode && (
                <div style={{display: 'inline-flex',alignItems: 'center',gap: 6,padding: '5px 12px',borderRadius: 999,marginBottom: 12,background: `${modeColors[marketMode] || '#888'}08`,border: `1px solid ${modeColors[marketMode] || '#888'}15`}}>
                    <div style={{width: 6,height: 6,borderRadius: '50%',background: modeColors[marketMode] || '#888'}} />
                    <span style={{fontSize: 9,fontWeight: 900,color: modeColors[marketMode] || '#888',textTransform: 'uppercase',letterSpacing: '0.1em'}}>
                        {marketMode.replace('_', ' ')}
                    </span>
                </div>
            )}

            <h1 style={{fontSize: 'clamp(1.8rem, 4vw, 2.6rem)',fontWeight: 900,lineHeight: 1.08,letterSpacing: '-0.03em',margin: 0,color: '#fff'}}>
                {title}
            </h1>
            <p style={{fontSize: 14,color: 'rgba(255,255,255,0.4)',lineHeight: 1.5,marginTop: 8,maxWidth: 600}}>
                {subtitle}
            </p>
        </div>
    );
