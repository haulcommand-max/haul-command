...
serve(async (req) => {
  ...
  const importRecord = await resolveImport({ ...body, source_format: "json" });
  ...
  try {
    ...
    await updateImport(importRecord.id as string | null, {
      ...
      result_summary: { parser: "authority-json-parser", observations: observations.length, stored },
    });

    return json(200, { ok: true, parser: "authority-json-parser", observations: observations.length, stored });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await updateImport(importRecord.id as string | null, { status: "failed", last_error: message });
    return json(500, { ok: false, parser: "authority-json-parser", error: message });
  }
});
