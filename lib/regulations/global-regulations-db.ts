...
export const REGULATIONS: CountryRegulation[] = [
    ...
    ...["UY", "PA", "CR"].map(code => ({
        countryCode: code,
        countryName: code === "UY" ? "Uruguay" : code === "PA" ? "Panama" : "Costa Rica",
        tier: "D" as const,
        terminology: { primary: "vehículo escolta", language: "es" },
        standardLimits: { widthM: 2.60 },
        escortThresholds: [
        ...
        permitSystem: { authority: "National road/transport authority" },
        equipment: ["Warning signs", "Amber lights"],
        dataQuality: "low" as const,
        voiceAnswer: `In ${code === "UY" ? "Uruguay" : code === "PA" ? "Panama" : "Costa Rica"}, oversize loads typically require permits. Escorts may be needed when width exceeds about 3 meters. Police escorts may be required for very large loads. Contact local authorities for specific requirements.`,
    })),
];
