...
// Corridor catalog — 35 corridors, each a $199/month sponsorship opportunity
const CORRIDORS: Record<string, {
    ...
    'ms-al': { name: 'Mississippi → Alabama', state1: 'MS', state2: 'AL', miles: 175, avgRate: '$3.20–$4.60/mi', permits: 'Both states', escorts: 'Required both', riskGrade: 'C', notes: 'Manufacturing and port equipment. Mobile port approach active.', topHazards: ['Mobile Bay bridges', 'Tuscaloosa metro'] },
    'al-ga': { name: 'Alabama → Georgia', state1: 'AL', state2: 'GA', miles: 200, avgRate: '$3.30–$4.70/mi', permits: 'Both states', escorts: 'Required both', riskGrade: 'C', notes: 'Auto parts and manufacturing moves. I-20 primary route.', topHazards: ['Atlanta urban restrictions', 'Birmingham staging'] },
    'tx-nm': { name: 'Texas → New Mexico', state1: 'TX', state2: 'NM', miles: 310, avgRate: '$3.60–$5.00/mi', permits: 'Both states', escorts: 'Required TX', riskGrade: 'C', notes: 'Oil and gas equipment. Long-haul desert corridor with few services.', topHazards: ['El Paso urban access', 'High desert wind restrictions'] },
    // ─── Southeast ───────────────────────────────────────────
    ...
    'ga-sc': { name: 'Georgia → South Carolina', state1: 'GA', state2: 'SC', miles: 155, avgRate: '$3.20–$4.50/mi', permits: 'Both states', escorts: 'Required both', riskGrade: 'C', notes: 'Automotive and manufacturing moves. BMW and Mercedes plants active corridor.', topHazards: ['Augusta bridge weight', 'SC port approach'] },
    'sc-nc': { name: 'South Carolina → North Carolina', state1: 'SC', state2: 'NC', miles: 200, avgRate: '$3.10–$4.40/mi', permits: 'Both states', escorts: 'Required both', riskGrade: 'C', notes: 'Manufactured housing and construction equipment. High-volume year-round.', topHazards: ['Charlotte metro', 'Columbia urban access'] },
    'nc-va': { name: 'North Carolina → Virginia', state1: 'NC', state2: 'VA', miles: 230, avgRate: '$3.20–$4.60/mi', permits: 'Both states', escorts: 'Required both', riskGrade: 'C', notes: 'Port equipment and lumber. Norfolk port primary destination.', topHazards: ['Richmond metro bridges', 'Hampton Roads tunnels'] },
    // ─── Mid-Atlantic ─────────────────────────────────────────
    'va-md': { name: 'Virginia → Maryland', state1: 'VA', state2: 'MD', miles: 140, avgRate: '$3.50–$5.20/mi', permits: 'Both + DC', escorts: 'Required both', riskGrade: 'A', notes: 'Highest permit complexity. DC perimeter restrictions apply for oversized loads.', topHazards: ['DC Capitol restrictions', 'Bay Bridge load limits', 'Baltimore tunnel ban'] },
    'md-pa': { name: 'Maryland → Pennsylvania', state1: 'MD', state2: 'PA', miles: 165, avgRate: '$3.40–$5.00/mi', permits: 'MDSHA + PennDOT', escorts: 'Required both', riskGrade: 'B', notes: 'Steel and manufacturing loads. PennDOT known for strict enforcement.', topHazards: ['PennDOT night ban', 'Baltimore inner harbor access'] },
    ...
    'pa-nj': { name: 'Pennsylvania → New Jersey', state1: 'PA', state2: 'NJ', miles: 130, avgRate: '$3.80–$5.50/mi', permits: 'PennDOT + NJDOT', escorts: 'Required both', riskGrade: 'A', notes: 'Port Newark corridor. NJ has strict equipment standards and limited move windows.', topHazards: ['Delaware River crossings', 'NJ Turnpike overhead signs', 'Port restricted hours'] },
    'nj-ny': { name: 'New Jersey → New York', state1: 'NJ', state2: 'NY', miles: 120, avgRate: '$4.50–$7.00/mi', permits: 'NJDOT + NYSDOT + Port Auth', escorts: 'Police required', riskGrade: 'A', notes: 'Highest complexity in US. Port Authority permits required for bridge/tunnel crossing. NYPD escort mandatory above certain dimensions.', topHazards: ['GWB height limit 13.5ft', 'Lincoln Tunnel ban', 'NYSDOT weekend restrictions'] },
    // ─── Midwest ──────────────────────────────────────────────
    'oh-in': { name: 'Ohio → Indiana', state1: 'OH', state2: 'IN', miles: 160, avgRate: '$3.00–$4.40/mi', permits: 'Both states', escorts: 'Required OH', riskGrade: 'C', notes: 'High manufacturing volume. Industrial equipment and modular buildings.', topHazards: ['Indianapolis metro', 'Fort Wayne industrial zones'] },
    'in-il': { name: 'Indiana → Illinois', state1: 'IN', state2: 'IL', miles: 175, avgRate: '$3.10–$4.50/mi', permits: 'Both states', escorts: 'Required both', riskGrade: 'B', notes: 'Chicago metro approach — most complex Midwest routing. Night windows often required.', topHazards: ['Chicago metro weight ordinance', 'Cook County restrictions', 'I-80 toll plaza widths'] },
    'il-mo': { name: 'Illinois → Missouri', state1: 'IL', state2: 'MO', miles: 185, avgRate: '$3.00–$4.40/mi', permits: 'IDOT + MoDOT', escorts: 'Required both', riskGrade: 'C', notes: 'Bridge and manufacturing equipment. Mississippi River crossings key.', topHazards: ['St. Louis metro bridges', 'Mississippi River weight'] },
    'mo-ks': { name: 'Missouri → Kansas', state1: 'MO', state2: 'KS', miles: 210, avgRate: '$2.80–$4.20/mi', permits: 'Both states', escorts: 'Not required', riskGrade: 'D', notes: 'Wind farm and agricultural equipment. Flat open corridor, lower complexity.', topHazards: ['Kansas City metro', 'I-70 construction zones'] },
    'mi-oh': { name: 'Michigan → Ohio', state1: 'MI', state2: 'OH', miles: 145, avgRate: '$3.10–$4.50/mi', permits: 'Both states', escorts: 'Required MI', riskGrade: 'C', notes: 'Auto industry components. Ford, GM, Stellantis corridors heavily used.', topHazards: ['Toledo urban access', 'Detroit river bridge'] },
    // ─── Plains ───────────────────────────────────────────────
    'tx-ks': { name: 'Texas → Kansas', state1: 'TX', state2: 'KS', miles: 380, avgRate: '$3.20–$4.70/mi', permits: 'Both states', escorts: 'Required TX', riskGrade: 'C', notes: 'Wind turbine and agricultural equipment. Long flat corridor, high volume.', topHazards: ['Oklahoma City bypass', 'Wichita metro'] },
    'ks-ne': { name: 'Kansas → Nebraska', state1: 'KS', state2: 'NE', miles: 200, avgRate: '$2.80–$4.10/mi', permits: 'Both states', escorts: 'Not required', riskGrade: 'D', notes: 'Wind and agricultural. One of the simplest corridors nationally.', topHazards: ['Topeka-Omaha urban stretches'] },
    'nd-sd': { name: 'North Dakota → South Dakota', state1: 'ND', state2: 'SD', miles: 220, avgRate: '$2.90–$4.30/mi', permits: 'Both states', escorts: 'Not required', riskGrade: 'D', notes: 'Oil and wind equipment. Low population, high loads.', topHazards: ['Fargo city limits', 'Seasonal frost laws'] },
    // ─── Mountain ─────────────────────────────────────────────
    ...
    'wy-co': { name: 'Wyoming → Colorado', state1: 'WY', state2: 'CO', miles: 260, avgRate: '$3.10–$4.60/mi', permits: 'Both states', escorts: 'Required CO', riskGrade: 'B', notes: 'Mining and oil equipment. CDOT strict on mountain passes.', topHazards: ['Denver metro', 'I-25 mountain grade', 'CDOT night ban on Eisenhower'] },
    'co-ut': { name: 'Colorado → Utah', state1: 'CO', state2: 'UT', miles: 290, avgRate: '$3.20–$4.80/mi', permits: 'CDOT + UDOT', escorts: 'Required both', riskGrade: 'B', notes: 'Mining and construction equipment. Mountain passes frequent.', topHazards: ['Salt Lake metro', 'I-70 mountain grades', 'UDOT weekend restrictions'] },
    'ut-nv': { name: 'Utah → Nevada', state1: 'UT', state2: 'NV', miles: 310, avgRate: '$3.00–$4.50/mi', permits: 'Both states', escorts: 'Not required', riskGrade: 'C', notes: 'Mining equipment and modular buildings. Desert conditions limit move windows.', topHazards: ['Las Vegas metro', 'Salt flats weight limits', 'Summer heat restrictions'] },
    'id-mt': { name: 'Idaho → Montana', state1: 'ID', state2: 'MT', miles: 290, avgRate: '$3.00–$4.40/mi', permits: 'Both states', escorts: 'Not required', riskGrade: 'C', notes: 'Mining and forest equipment. Remote corridor with limited service stops.', topHazards: ['Lolo Pass restrictions', 'Helena metro'] },
    // ─── West Coast ───────────────────────────────────────────
    ...
    'or-wa': { name: 'Oregon → Washington', state1: 'OR', state2: 'WA', miles: 180, avgRate: '$3.50–$5.10/mi', permits: 'ODOT + WSDOT', escorts: 'Required both', riskGrade: 'B', notes: 'Wind turbine and forest equipment. Columbia River crossings critical.', topHazards: ['Columbia River gorge', 'Portland metro bridges'] },
    'wa-or': { name: 'Washington → Oregon', state1: 'WA', state2: 'OR', miles: 180, avgRate: '$3.50–$5.10/mi', permits: 'WSDOT + ODOT', escorts: 'Required both', riskGrade: 'B', notes: 'Same corridor reverse. Same hazards, different permit order.', topHazards: ['Columbia River gorge', 'Portland metro bridges'] },
    'ca-nv': { name: 'California → Nevada', state1: 'CA', state2: 'NV', miles: 280, avgRate: '$3.80–$5.50/mi', permits: 'Caltrans + NDOT', escorts: 'Caltrans strict', riskGrade: 'A', notes: 'Solar and mining equipment. Caltrans rules govern the most complex portion.', topHazards: ['Cajon Pass grade', 'CA night ban', 'Mojave heat windows'] },
    // ─── Canada ───────────────────────────────────────────────
    'ab-sk': { name: 'Alberta → Saskatchewan', state1: 'AB', state2: 'SK', miles: 330, avgRate: 'CAD $3.50–$5.00/mi', permits: 'Alberta + Sask', escorts: 'Required AB', riskGrade: 'C', notes: 'Oil sands equipment and wind turbines. Alberta has its own pilot car certification (PCP).', topHazards: ['Edmonton metro', 'Highway 1 load limits', 'Winter frost law season'] },
    'on-qc': { name: 'Ontario → Quebec', state1: 'ON', state2: 'QC', miles: 260, avgRate: 'CAD $3.80–$5.40/mi', permits: 'MTO + MTQ', escorts: 'Required both', riskGrade: 'B', notes: 'Industrial corridor. Quebec requires French-language permits and has strict SAAQ escort rules.', topHazards: ['Montreal urban', 'Ottawa River bridges', 'SAAQ language requirements'] },
};
