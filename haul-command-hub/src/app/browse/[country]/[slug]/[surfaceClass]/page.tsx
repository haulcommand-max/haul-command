...
export async function generateMetadata({ params }: { params: Promise<{ country: string; slug: string; surfaceClass: string }> }) {
...
export default async function CityClassPage({ params }: { params: Promise<{ country: string; slug: string; surfaceClass: string }> }) {
    const { country, slug: city, surfaceClass } = await params;
