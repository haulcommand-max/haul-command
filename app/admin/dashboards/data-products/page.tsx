...
// Simulated purchase data until real data flows
...
const TIER_LABELS: Record<string, { label: string; color: string; flag: string }> = {
    A: { label: 'Gold', color: '#C6923A', flag: '🥇' },
    B: { label: 'Blue', color: '#3B82F6', flag: '🔵' },
    C: { label: 'Silver', color: '#9CA3AF', flag: '🥈' },
    D: { label: 'Slate', color: '#6B7280', flag: '⬜' },
};
...
export default function DataProductsDashboard() {
    ...
    return (
        <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto', fontFamily: "var(--font-body, 'Inter', sans-serif)" }}>
            ...
            {/* Pricing Tab — 57-Country Tier Grid */}
            {activeTab === 'pricing' && (
                <div style={{
                    padding: 24, borderRadius: 16,
                    border: '1px solid rgba(255,255,255,0.06)',
                    ...
                    {Object.entries({
                        ...
                        return (
                            <div key={tier} style={{ marginBottom: 20 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                                    <span style={{ fontSize: 14, fontWeight: 900, color: meta.color }}>
                                        {meta.flag} Tier {tier} — {meta.label}
                                    </span>
                                ...
                                <div style={{ marginTop: 8, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                                    {products.filter(p => p.price_usd > 0 && p.active).slice(0, 4).map(p => (
                                        <span key={p.id} style={{ fontSize: 10, color: '#888' }}>
                                            {p.name}: <strong style={{ color: meta.color }}>${Math.round(p.price_usd * multiplier)}</strong>
                                        </span>
                                    ))}
                                </div>
                            </div>
                    ...
                    {products.map(p => (
                        <div key={p.id} style={{
                            ...
                            <div style={{
                                ...
                                <div>
                                    <div style={{ fontSize: 9, color: '#666', textTransform: 'uppercase', marginBottom: 2 }}>Price</div>
                                    <div style={{ fontSize: 18, fontWeight: 900, color: '#C6923A' }}>
                                        {p.price_usd === 0 ? 'Free*' : `$${p.price_usd}`}
                                        {p.purchase_type === 'subscription' && <span style={{ fontSize: 10, color: '#888' }}>/mo</span>}
                                    </div>
                                </div>
                                <div>
                                    <div style={{ fontSize: 9, color: '#666', textTransform: 'uppercase', marginBottom: 2 }}>Purchases</div>
                                    <div style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>{Math.floor(Math.random() * 100)}</div>
                                </div>
                                <div>
                                    <div style={{ fontSize: 9, color: '#666', textTransform: 'uppercase', marginBottom: 2 }}>Revenue</div>
                            ...
                            <div style={{
                                ...
                                <div style={{ fontSize: 9, color: '#888' }}>
                                    <Globe size={10} style={{ display: 'inline', marginRight: 3, verticalAlign: 'middle' }} />
                                    {p.country_scope.includes('ALL') ? 'All 57' : `${p.country_scope.length} countries`}
                                </div>
                            </div>
                        </div>
                    ))}
