...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Heavy haul escort intelligence for the ${corridor.origin_state} to ${corridor.destination_state} corridor. Permit requirements, escort regulations, operator density, and real traffic data.`,
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <section className="py-12 px-4 border-b border-white/5">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs text-gray-400">{corridor.origin_state} to {corridor.destination_state}</span>
          </div>
          <div className="flex items-center gap-4 mb-3">
            <h1 className="text-3xl md:text-4xl font-bold">
              {corridor.origin_state} to {corridor.destination_state} Escort Corridor
            </h1>
          ...
          <div className="flex gap-6 mt-4 text-sm">
            <div><span className="text-amber-400 font-bold">{corridor.load_count ?? 0}</span><span className="text-gray-600 ml-1">loads posted</span></div>
            <div><span className="text-amber-400 font-bold">{corridor.operator_count ?? 0}</span><span className="text-gray-600 ml-1">active escorts</span></div>
            <div><span style={{ color: riskColors[routeIntel.riskGrade] }} className="font-bold">Grade {routeIntel.riskGrade}</span><span className="text-gray-600 ml-1">risk</span></div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            ...
            <div className="mb-8 p-5 bg-white/[0.03] border border-white/10 rounded-2xl">
              <h2 className="text-sm font-bold text-amber-400 uppercase tracking-widest mb-4">Route Intelligence (Computed)</h2>
              ...
              <div className="mt-4 p-3 bg-green-500/5 border border-green-500/15 rounded-lg">
                ...
                {routeIntel.recommendations.map((r, i) => <div key={i} className="text-xs text-gray-400 mb-1">check {r}</div>)}
              </div>
            </div>

            {corridor.intel_content ? (
              <div className="prose prose-invert prose-sm max-w-none prose-headings:text-white prose-headings:font-bold prose-p:text-gray-400 prose-p:leading-relaxed prose-strong:text-white prose-a:text-amber-400">
              ...
              <div className="p-6 bg-white/5 border border-white/10 rounded-2xl text-center">
                ...
                <a href="/route-check" className="text-amber-400 text-sm hover:underline mt-2 inline-block">Use Route Check for instant answers</a>
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
              ...
              {operators?.length ? (
                <div className="space-y-2">
                  ...
                  <a href={`/directory?state=${origin.toUpperCase()}`} className="block text-center text-xs text-amber-400 hover:underline mt-2">
            ...
            <PostLoadCTA corridorName={`${corridor.origin_state} to ${corridor.destination_state}`} variant="card" />
            <OperatorsNeededCTA
            ...
            <DataProductTeaser
              ...
              description={`${corridor.origin_state} to ${corridor.destination_state} demand, supply, and rate data`}
              previewData={[
                `Operators active: ${corridor.operator_count || '-'}`,
                `Loads posted (24h): ${corridor.load_count || '-'}`,
                `Avg rate per mile: (unlocked)`,
              ]}
              price="$29/mo"
        ...
        {/* SEO Internal Links — corridor pages flow equity to escort-calc, directory, requirements, loads */}
        <RelatedLinks
          pageType="corridor"
          heading={`Related resources for the ${corridor.origin_state} to ${corridor.destination_state} corridor`}
          className="mt-10"
        />
      </div>
    </div>
