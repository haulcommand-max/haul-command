...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Escort Rates in ${stateName}, ${cc.name} — HAUL COMMAND`,
    description: `Rate benchmarks for escort and pilot car services in ${stateName}, ${cc.name}.`,
  };
}
...
export default async function RatesStatePage({ params }: Props) {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8 min-h-screen">
      ...
      <HCLocalIntroCopy h1={`Escort Rates in ${stateName}`} intro={`Rate intelligence for ${stateName}, ${cc.name}. Data is being collected.`} badge={`${cc.flag} ${stateName}`} />
      <HCAlertSignupModule context={`${stateName} rate data`} />
    </main>
  );
}
