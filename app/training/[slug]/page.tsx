...
export async function generateStaticParams() {
  ...
  return (data || []).map((m: any) => ({ slug: m.module_slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  
  if (COUNTRY_CODES.has(slug.toLowerCase())) {
     return {
        title: `Compliance Operations in ${slug.toUpperCase()} | Haul Command`,
        description: `Country-specific training overlay for ${slug.toUpperCase()}`,
  ...
  const title = mod?.module_title ?? slug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
...
export default async function TrainingSlugPage({ params }: Props) {
  ...
  if (COUNTRY_CODES.has(slug.toLowerCase())) {
    return <CountryTrainingView country={slug} />;
  }
  ...
  const schema = mod ? [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: mod.module_title, item: `https://www.haulcommand.com/training/${slug}` },
      ],
    }
  ] : null;
    ...
    <>
      ...
      <nav aria-label="Breadcrumb" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '12px 0', background: '#07090d' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 20px', display: 'flex', gap: 6, fontSize: 11, color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
          ...
          <span>›</span>
          <span style={{ color: gold }}>{mod?.module_title ?? slug}</span>
        </div>
      </nav>
        ...
        <header style={{ background: '#07090d', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: 'clamp(1.5rem,4vw,3rem) 20px' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <Link href={`/training/${slug}#start`} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 24px', borderRadius: 12, background: `linear-gradient(135deg, ${gold}, #E4B872)`, color: '#000', fontSize: 14, fontWeight: 900, textDecoration: 'none' }}>
                🎓 Access Module
              </Link>
      ...
      <ModuleDetail params={Promise.resolve({ module_slug: slug })} />
