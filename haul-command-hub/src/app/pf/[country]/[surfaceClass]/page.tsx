...
type Props = { params: Promise<{ country: string; surfaceClass: string }> };

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { country, surfaceClass } = await params;
    const pk = await getPageKey('country_class', { country_slug: country, surface_class: surfaceClass });
...
export default async function CountryClassPage({ params }: Props) {
    ...
    const [surfaces, links, cities] = await Promise.all([
        getSurfaces({ country_code: pk.country_code!, surface_class: surfaceClass }, 60),
        getInternalLinks(pk.id),
        getCityRollups(pk.country_code!, surfaceClass),
    ]);
                ...
                <section style={{ marginBottom: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                        {cities.slice(0, 15).map((c: any) => (
                            <Link
                                key={c.city}
                                href={`/pf/${country}/${c.city?.toLowerCase().replace(/\s+/g, '-')}/${surfaceClass}`}
                                style={{
