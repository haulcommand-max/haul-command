...
export async function GET() {
  try {
    ...
    return NextResponse.json({ countries: data?.length ? data : FALLBACK_COUNTRIES, source: data?.length ? 'supabase' : 'fallback' });
  } catch (err) {
...
const FALLBACK_COUNTRIES = [
  ...
  { country_code: 'SA', country_name: 'Saudi Arabia', max_routine_width_meters: 3.75, max_routine_height_meters: 4.8, max_routine_length_meters: 22.0, max_routine_weight_kg: 66000, escort_required_width_meters: 3.75, standard_curfews_apply: true, night_moves_allowed: true, metric_standard: true, transport_authority_notes: 'TGA regulates. Desert corridor moves common at night.' },
