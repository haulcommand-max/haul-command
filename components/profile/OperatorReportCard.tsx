// components/profile/OperatorReportCard.tsx
// Performance stats from hc_operator_report_card
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-[#0c1015] last:border-0">
      ...
      <div className="text-right">
        <span className="text-sm font-semibold text-[#d0dce8]">{value}</span>
        {sub && <p className="text-[10px] text-[#3a5068]">{sub}</p>}
      </div>
    </div>
  );
}
...
export function OperatorReportCard({ reportCard: rc, operator }: ReportCardProps) {
  ...
  const rankTitle = rc?.rank_title ?? 'Operator';
  const rankLevel = rc?.rank_level;
  ...
  return (
    <div className="bg-[#0f1a24] border border-[#1e3048] rounded-2xl p-6">
      <div className="flex items-center justify-between mb-5">
        ...
        {rankTitle && (
          <span className="text-[10px] text-[#d4950e] font-bold">
            {rankTitle}{rankLevel ? ` · L${rankLevel}` : ''}
          </span>
        )}
      </div>
      ...
      {rc?.signals_reported > 0 && (
        <StatRow label="CSN Signals Filed" value={rc.signals_reported.toString()} sub={`${Math.round(Number(rc.signal_accuracy || 0) * 100)}% accuracy`} />
      )}
      {rc?.safety_incidents === 0 && (
        <div className="mt-3 text-xs text-[#22c55e] bg-[#0d2000] border border-[#2a5010] rounded-lg px-3 py-2">
          ✓ Zero safety incidents recorded
        </div>
      )}

      <div className="mt-4 pt-4 border-t border-[#0c1015]">
        <p className="text-[10px] text-[#3a4e64]">
          Member since {new Date(operator.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
        </p>
      </div>
    </div>
  );
