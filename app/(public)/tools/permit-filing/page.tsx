      ...
      <div style={{background: 'linear-gradient(135deg, #07090f, #0d1422)',borderBottom: '1px solid #1a1a22',padding: '48px 24px 40px' }}>
        <div style={{maxWidth: 800,margin: '0 auto',textAlign: 'center' }}>
          <div style={{display: 'inline-block',padding: '5px 14px',borderRadius: 16,background: 'rgba(245,166,35,0.1)',border: '1px solid rgba(245,166,35,0.25)',color: '#F5A623',fontSize: 11,fontWeight: 700,marginBottom: 20,letterSpacing: '0.1em' }}>
            PERMIT QUOTE WORKFLOW
          </div>
          <h1 style={{fontSize: 'clamp(26px, 4vw, 40px)',fontWeight: 900,margin: '0 0 16px',lineHeight: 1.1 }}>
            Price and Start Oversize Permit Filing
          </h1>
          <p style={{color: '#8a8aa0',fontSize: 16,maxWidth: 560,margin: '0 auto 0' }}>
            Select states, enter your load dimensions, and generate a checkout-backed filing request.
            Final issuance, processing time, and required attachments still depend on each jurisdiction.
          </p>
        </div>
      </div>
      ...
      <div style={{maxWidth: 800,margin: '0 auto',padding: '40px 24px' }}>
        ...
        {step === 'configure' && (
          <>
            ...
            <div style={{background: '#111118',border: '1px solid #1a1a22',borderRadius: 16,padding: 28,marginBottom: 20 }}>
              ...
              {selectedStates.length > 0 && (
                <div style={{marginTop: 12,fontSize: 13,color: '#6a6a7a' }}>
                  ...
                  {selectedStates.length >= 3 && <span style={{color: '#22c55e',marginLeft: 8 }}>Multi-state pricing applied</span>}
                </div>
              )}
            </div>
            ...
            <div style={{background: '#111118',border: '1px solid #1a1a22',borderRadius: 16,padding: 28,marginBottom: 20 }}>
              ...
              <div style={{display: 'flex',gap: 10,flexWrap: 'wrap' }}>
                {[
                  ...
                  <button aria-label="Interactive Button" data-tool-interact key={opt.val} onClick={() => setPermitType(opt.val)} style={{padding: '12px 18px',borderRadius: 10,cursor: 'pointer',background: permitType === opt.val ? `rgba(${opt.color === '#3b82f6' ? '59,130,246' : opt.color === '#F5A623' ? '245,166,35' : '168,85,247'},0.15)` : '#0c0c10',border: `1px solid ${permitType === opt.val ? opt.color : '#2a2a3a'}`,color: permitType === opt.val ? opt.color : '#8a8aa0',textAlign: 'left'}}>
              ...
              <label style={{display: 'flex',alignItems: 'center',gap: 10,marginTop: 16,cursor: 'pointer' }}>
                <input data-tool-interact type="checkbox" checked={rush} onChange={e => setRush(e.target.checked)}
            ...
            <button aria-label="Interactive Button" data-tool-interact onClick={getQuote} disabled={!selectedStates.length || loading} style={{width: '100%',padding: 16,background: selectedStates.length
                ? 'linear-gradient(135deg, #F5A623, #e08820)'
                : '#1a1a22',color: selectedStates.length ? '#000' : '#5a5a6a',border: 'none',borderRadius: 12,fontSize: 16,fontWeight: 800,cursor: selectedStates.length ? 'pointer' : 'not-allowed'}}>
          ...
          <div style={{background: '#111118',border: '1px solid #1a1a22',borderRadius: 16,padding: 28 }}>
            ...
            <div style={{marginBottom: 20 }}>
              {quote.line_items.map(item => (
                <div key={item.state} style={{display: 'flex',justifyContent: 'space-between',alignItems: 'center',padding: '12px 0',borderBottom: '1px solid #1a1a22',fontSize: 14}}>
                  <div>
            ...
            <div style={{background: 'rgba(34,197,94,0.08)',border: '1px solid rgba(34,197,94,0.2)',borderRadius: 10,padding: 14,marginBottom: 24,fontSize: 13 }}>
              Estimated filing request handling: <strong style={{color: '#22c55e' }}>{quote.estimated_processing_days} business day{quote.estimated_processing_days !== 1 ? 's' : ''}</strong>.
              State approval and final permit delivery depend on the issuing authority.
            </div>

            <div style={{display: 'flex',gap: 12 }}>
              <button aria-label="Interactive Button" data-tool-interact onClick={() => setStep('configure')} style={{padding: '12px 20px',borderRadius: 10,background: '#0c0c10',border: '1px solid #2a2a3a',color: '#8a8aa0',fontSize: 14,cursor: 'pointer'}}>
                Back to edit
              </button>
              <button aria-label="Interactive Button" data-tool-interact onClick={checkout} disabled={loading} style={{flex: 1,padding: 14,background: 'linear-gradient(135deg, #F5A623, #e08820)',color: '#000',border: 'none',borderRadius: 10,fontSize: 15,fontWeight: 800,cursor: 'pointer'}}>
                {loading ? 'Redirecting to checkout...' : 'Pay and Submit Filing Request ->'}
              </button>
            </div>
          </div>
        )}
