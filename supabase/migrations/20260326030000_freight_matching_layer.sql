-- ==============================================================================
-- HAUL COMMAND: PHASE 2 - FREIGHT MATCHING & ESCROW LAYER
-- Integrates Brokers and Loads directly with the 1.56M Directory Listings.
-- ==============================================================================
...
CREATE POLICY "Public broker profile visibility"
ON public.broker_profiles FOR SELECT USING (true);

CREATE POLICY "Brokers can update own profile"
ON public.broker_profiles FOR UPDATE USING (auth.uid() = user_id);
...
CREATE TABLE IF NOT EXISTS public.loads (
  ...
  load_type text NOT NULL,                  -- superload, heavy_haul, wide_load
  required_services text[] DEFAULT '{}',    -- Maps to directory_listings metadata->services
  
  -- Financial
  price_cents integer NOT NULL,             -- Always store money in integers (cents) to avoid rounding
...
ON public.loads FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Brokers can insert and manage their own loads"
ON public.loads FOR ALL USING (
  broker_id IN (SELECT id FROM public.broker_profiles WHERE user_id = auth.uid())
);

-- 4. LOAD BIDS (Operator Marketplace Competition)
-- This allows operators from your directory to bid on available loads.
...
CREATE POLICY "Brokers see bids on their loads, Operators see their own bids"
ON public.load_bids FOR SELECT USING (
...
-- Holds funds safely until delivery is confirmed by both parties.
...
CREATE POLICY "Participants can view their own escrow transactions"
ON public.escrow_payments FOR SELECT USING (
