        ...
        <><Navbar />
            <main className="flex-grow max-w-5xl mx-auto px-4 py-16">
                <header className="mb-16 text-center">
                    ...
                    <h1 className="text-4xl md:text-6xl font-black text-white italic tracking-tighter mt-6">
                        ESCORT <span className="text-accent underline decoration-4 underline-offset-4">COMPLIANCE CARD</span>
                    </h1>
                    <p className="text-gray-400 text-lg max-w-2xl mx-auto mt-4">
                        One-page reference card with every dimension threshold, escort configuration,
                        required equipment, and authority contact. Always current. Always free.
                    </p>
                </header>
                ...
                {!submitted ? (
                    <div className="max-w-2xl mx-auto">
                        ...
                        <div className={`bg-white/5 border border-white/10 rounded-[32px] p-8 mb-6 ${step !== 1 ? 'opacity-50' : ''}`}>
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <p className="text-white font-black">Select Your State or Country</p>
                            </div>
                            {step === 1 && (
                                <>
                                    <div className="mb-4">
                                        <p className="text-gray-500 text-[10px] font-black uppercase mb-3">US States</p>
                                        <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto">
                                            {US_STATES.map(s => (
                                                <button key={s.code} onClick={() => setSelected(s.code)}
                                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${selected === s.code ? 'bg-accent text-black' : 'bg-white/5 text-gray-400 hover:bg-white/10'
                                                        }`}>{s.name}</button>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="mb-4">
                                        <p className="text-gray-500 text-[10px] font-black uppercase mb-3">International</p>
                                    ...
                                    <button onClick={() => setStep(2)}
                                        className="w-full bg-white text-black py-3 rounded-xl font-black text-sm hover:bg-accent transition-all mt-4">
                                        GET {selectedName.toUpperCase()} CARD →
                                    </button>
                                </>
                            )}
                        </div>
                        ...
                        <div className={`bg-white/5 border border-white/10 rounded-[32px] p-8 ${step !== 2 ? 'opacity-50 pointer-events-none' : ''}`}>
                            <div className="flex items-center gap-3 mb-6">
                                <span className="w-8 h-8 rounded-full bg-accent text-black flex items-center justify-center font-black text-sm">2</span>
                                <p className="text-white font-black">Where Should We Send It?</p>
                            </div>
                            {step === 2 && (
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-accent/10 border border-accent/20 rounded-xl p-4">
                                        <p className="text-accent font-black text-sm">What you&apos;ll get:</p>
                                        <ul className="text-gray-400 text-xs mt-2 space-y-1">
                                            <li>✓ {selectedName} Escort Compliance Card (PDF)</li>
                                            <li>✓ All dimension thresholds on one page</li>
                                            <li>✓ Authority contacts &amp; official links</li>
                                            <li>✓ Regulation change alerts for {selectedName}</li>
                                        </ul>
                                    </div>
                                    <button onClick={handleSubmit} disabled={!email || loading}
                                        className="w-full bg-accent text-black py-4 rounded-xl font-black text-lg hover:bg-white transition-all disabled:opacity-50">
                                        {loading ? 'SENDING...' : `SEND MY FREE ${selectedName.toUpperCase()} CARD`}
                                    </button>
                                    <p className="text-gray-600 text-[10px] text-center">No spam. Unsubscribe anytime. We respect your inbox.</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="max-w-2xl mx-auto text-center">
                        <div className="bg-green-500/10 border border-green-500/20 rounded-[32px] p-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                                <a href="/tools/escort-calculator"
                                    ...
                                    <p className="text-accent font-black text-sm">🧮 ROUTE CALCULATOR</p>
                                    <p className="text-gray-400 text-xs mt-1">Check escorts for your entire route</p>
                                </a>
                                <a href="/directory"
                                    className="bg-white/5 border border-white/10 rounded-xl p-6 hover:border-accent/50 transition-all text-left">
