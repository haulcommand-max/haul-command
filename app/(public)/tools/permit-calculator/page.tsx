                    ...
                    <div style={{background: T.panel,border: `1px solid ${T.border}`,borderRadius: 20,padding: 24 }}>
                        ...
                        <div style={{display: 'flex',flexWrap: 'wrap',gap: 6,maxHeight: 260,overflowY: 'auto' }}>
                            {US_STATES.map(({ code, name }) => {
                                ...
                                return (
                                    <button aria-label="Interactive Button" key={code} onClick={() => toggleState(code)} style={{padding: '6px 12px',borderRadius: 8,fontSize: 12,fontWeight: 700,cursor: 'pointer',transition: 'all 0.12s',background: active ? 'rgba(245,185,66,0.15)' : 'rgba(255,255,255,0.03)',border: `1px solid ${active ? 'rgba(245,185,66,0.4)' : T.border}`,color: active ? T.gold : T.muted}}>
                                        {code}
                                    </button>
                                );
                            })}
