...
serve(async (req) => {
  ...
  const importRecord = await resolveImport({ ...body, source_format: "html_scrape" });
  ...
  try {
    ...
    const observations = extractHtmlObservations(html, sourceUrl);
    ...
    await updateImport(importRecord.id as string | null, {
      ...
      result_summary: { parser: "authority-html-scrape-parser", observations: observations.length, stored },
    });

    return json(200, { ok: true, parser: "authority-html-scrape-parser", observations: observations.length, stored });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await updateImport(importRecord.id as string | null, { status: "failed", last_error: message });
    return json(500, { ok: false, parser: "authority-html-scrape-parser", error: message });
  }
});
