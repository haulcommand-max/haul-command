...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title:`${metroName}, ${regionName} — Heavy Haul Directory`,
    description: `Find pilot car operators and escort services in the ${metroName} metro area, ${regionName}, ${cc.name}.`,
  };
}
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8 min-h-screen">
      ...
      <HCLocalIntroCopy
        ...
        intro={`Find pilot car and escort services in the ${metroName} area, ${regionName}, ${cc.name}. Data is being collected.`}
      ...
      <section className="mt-8 grid grid-cols-2 sm:grid-cols-3 gap-3">
        <Link href={`/directory/${country}/${state}`} className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-4 hover:border-accent/30 transition-all text-center">
          ...
          <p className="text-xs text-gray-300 mt-1">All {regionName}</p>
        </Link>
