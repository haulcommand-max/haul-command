 ...
 *   - hc_global_operators (real operators — fallback)
...
export async function getBrokerProfile(slug: string) {
  ...
  // Fallback to hc_global_operators (real verified data only)
  const { data: opBroker } = await sb.from('hc_global_operators')
    .select('*')
    .eq('slug', slug)
