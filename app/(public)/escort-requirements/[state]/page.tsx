...
function buildEscortRules(state: StateData): EscortRule[] {
    const rules: EscortRule[] = [
        { condition: `Width > ${state.maxWidthFt}ft`, requirement: "1 escort vehicle required (rear chase)", severity: "standard" },
        { condition: `Width > 14ft`, requirement: "Lead car + rear chase car required (2 escorts minimum)", severity: "warning" },
        { condition: `Height > ${state.maxHeightFt}ft`, requirement: "Height pole escort required — pole operator must survey clearances", severity: "warning" },
    ];
    if (state.policeWidthFt) {
        rules.push({ condition: `Width > ${state.policeWidthFt}ft`, requirement: `Law enforcement escort mandatory — 72h advance notice required`, severity: "critical" });
    }
    rules.push(
        { condition: "Night movement (typically after dusk)", requirement: "Illuminated OVERSIZE LOAD signs and amber rotating lights required on all escort vehicles", severity: "standard" },
        { condition: "Load exceeds 110,000 lbs GVW", requirement: "Bridge analysis required — route survey by licensed engineer", severity: "warning" },
        { condition: "Load spans multiple states", requirement: `${state.abbr} permit covers in-state movement only — separate permits required per state`, severity: "standard" }
    );
    return rules;
}
    ...
    return (
        <div className="overflow-hidden rounded-2xl border border-white/8">
            <table className="w-full" itemScope itemType="https://schema.org/Table">
                ...
                <tbody>
                    {rules.map((rule, i) => (
                        <tr key={i} className="border-b border-white/5 last:border-0" style={{ background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)" }}>
                            <td className="px-4 py-3 align-top">
                                <span className={`text-xs font-bold ${rule.severity === "critical" ? "text-red-400" : rule.severity === "warning" ? "text-amber-400" : "text-white/60"}`}>{rule.condition}</span>
                            </td>
                            <td className="px-4 py-3 align-top">
                                <div className="flex items-start gap-2">
                                    {rule.severity === "critical" ? <AlertTriangle className="w-3.5 h-3.5 text-red-400 flex-shrink-0 mt-0.5" /> : rule.severity === "warning" ? <AlertTriangle className="w-3.5 h-3.5 text-amber-400 flex-shrink-0 mt-0.5" /> : <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400/60 flex-shrink-0 mt-0.5" />}
                                    <span className="text-[11px] text-white/55 leading-snug">{rule.requirement}</span>
                                </div>
                            </td>
                        </tr>
                    ))}
                ...
                {items.map((item, i) => (
                    <div key={i} className="flex items-center gap-2.5">
                        ...
                        <span className={`text-[11px] ${item.required ? "text-white/60" : "text-white/30"}`}>
                            {item.item}{!item.required && <span className="text-white/20 ml-1">(recommended)</span>}
                        </span>
                    </div>
                ))}
            </div>
                ...
                <table className="w-full" itemScope itemType="https://schema.org/Table">
                    ...
                    <tbody>
                        <tr className="border-b border-amber-500/15" style={{ background: "rgba(241,169,27,0.04)" }}>
                            ...
                            <td className="px-4 py-3 text-xs text-white/60 tabular-nums">{currentState.policeWidthFt ? `${currentState.policeWidthFt}ft+` : "Case-by-case"}</td>
                        </tr>
                        {compareStates.map((s, i) => (
                            <tr key={s.slug} className="border-b border-white/5 last:border-0" style={{ background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)" }}>
                                <td className="px-4 py-3">
                                    <Link aria-label="Navigation Link" href={`/escort-requirements/${s.slug}`} className="text-xs text-white/50 hover:text-amber-300 transition-colors font-medium">{s.name}</Link>
                                </td>
                                ...
                                <td className="px-4 py-3 text-xs text-white/40 tabular-nums">{s.policeWidthFt ? `${s.policeWidthFt}ft+` : "Case-by-case"}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <main className="min-h-screen" style={{ background: "#050505" }}>
                ...
                <div className="px-6 py-12 border-b border-white/5" style={{ background: "linear-gradient(135deg, rgba(241,169,27,0.03) 0%, transparent 60%)" }}>
                    <div className="max-w-5xl mx-auto">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-amber-400">{state.abbr} DOT Escort Regulations</span>
                        </div>
                ...
                <div className="border-b border-white/5 px-6 py-5" style={{ background: "rgba(255,255,255,0.02)" }}>
                    <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            { label: "No Escort Width", value: `${state.maxWidthFt}ft max`, color: "text-emerald-400" },
                            { label: "No Escort Height", value: `${state.maxHeightFt}ft max`, color: "text-emerald-400" },
                ...
                <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
                    <section>
                        <h2 className="text-base font-black text-white mb-4">Escort Requirements — {state.name} ({state.abbr})</h2>
                        ...
                        <div style={{ display: 'flex', gap: 10, marginTop: 16, flexWrap: 'wrap' }}>
                            ...
                            <Link href="/claim" style={{ padding: '10px 16px', borderRadius: 10, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', color: '#22C55E', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>Claim Free Listing</Link>
                            <Link href="/tools/escort-calculator" style={{ padding: '10px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#D1D5DB', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>Escort Calculator</Link>
                        </div>
                    </section>
                    ...
                    <section>
                        <h2 className="text-base font-black text-white mb-4">{state.name} Escort Rules — Common Questions</h2>
                        <div className="space-y-3">
                            {[
                                { q: `What are the oversize load escort requirements in ${state.name}?`, a: getRequirementsIntro(state) },
                                { q: `Does ${state.name} require pilot car certification?`, a: `${state.name} requires all pilot car operators to carry a valid driver's license and complete the ${state.abbr} DOT approved safety course.` },
                                { q: `What radio channel do pilot cars use in ${state.name}?`, a: `In ${state.name}, pilot car operators typically communicate on CB Channel 19, the standard nationwide pilot car channel.` },
                                { q: `Can a pilot car operate at night in ${state.name}?`, a: `Night moves in ${state.name} require additional lighting on all escort vehicles. The "OVERSIZE LOAD" sign must be illuminated, and amber rotating lights are mandatory.` },
                                { q: `How do I get an oversize load permit in ${state.name}?`, a: `Oversize permits in ${state.name} are issued by the ${state.abbr} Department of Transportation. Processing time is typically 1-3 business days.` },
                            ].map((faq, i) => (
                                <div key={i} className="rounded-xl border px-4 py-4" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.07)" }}>
                    ...
                    <section className="border-t border-white/5 pt-6">
                        <h3 className="text-xs font-black uppercase tracking-widest text-white/25 mb-3">Related Pages</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                            {[
                                { label: `Find Pilot Cars in ${state.name}`, href: `/find/pilot-car-operator/${state.slug}` },
                                ...state.topCities.slice(0, 3).map(c => ({ label: `Pilot Car ${c.name}`, href: `/find/pilot-car-operator/${state.slug}/${c.slug}` })),
                                ...state.corridors.slice(0, 2).map(c => ({ label: c.replace(/-/g, " ").toUpperCase(), href: `/corridors/${c}` })),
                                ...
                                <Link aria-label="Navigation Link" key={link.href} href={link.href} className="text-[11px] text-white/35 border border-white/8 rounded-lg px-3 py-2 hover:text-amber-300 hover:border-amber-500/25 transition-all text-center">{link.label}</Link>
                            ))}
                        </div>
                    </section>
                    ...
                    <NoDeadEndBlock
                        ...
                        moves={[
                            { href: `/find/pilot-car-operator/${state.slug}`, icon: 'search', title: `Find ${state.abbr} Operators`, desc: 'Verified, available now', primary: true, color: '#D4A844' },
                            { href: '/claim', icon: 'check', title: 'Claim Free Listing', desc: `For ${state.abbr} operators`, primary: true, color: '#22C55E' },
                            { href: `/market/${state.abbr.toLowerCase()}`, icon: 'chart', title: `${state.abbr} Market Intelligence`, desc: 'Active loads & supply data' },
                            { href: '/tools/escort-calculator', icon: 'calc', title: 'Escort Calculator', desc: 'Do I need an escort?' },
                            { href: '/escort-requirements', icon: 'law', title: 'All State Rules', desc: 'Compare across states' },
                            { href: '/glossary/oversize-load', icon: 'book', title: 'What Is an Oversize Load?', desc: 'Definitions & thresholds' },
                        ]}
                    />
                    ...
                    {/* SEO Internal Links — regulation pages flow equity to tools, directory, corridors */}
                    <RelatedLinks
                      pageType="regulation"
                      heading={`More ${state.name} heavy haul resources`}
                      className="mt-6"
                    />
                </div>
            </main>
