...
import { CATEGORIES } from '@/components/directory/CategoryGrid';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const categoryName = categoryConfig?.label || 'Operators';

  return {
    title: `${stateName} ${categoryName} | Haul Command`,
    description: `Find verified heavy haul ${categoryName.toLowerCase()} in ${stateName}. ${stateName} oversize load escorts with real ratings, permit experience, and instant contact.`,
  };
}

export default async function CategoryDirectoryPage({ params, searchParams }: Props) {
  const { state, country, category } = await params;
  const stateCode = state.toUpperCase();
  const stateName = STATE_NAMES[stateCode];
  
  const categoryConfig = CATEGORIES.find(c => c.slug === category);
  if (!stateName || !categoryConfig) notFound();
  ...
  // Map slugs to some backend filtering logic if necessary, or just serve the state directory and rely on search query
  if (category === 'pilot-car-operators') {
     query = query.or('entity_type.eq.pilot_car,role_primary.eq.pilot_car');
  }
  ...
  const breadcrumbData = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 4, "name": categoryName, "item": `https://haulcommand.com/directory/${country}/${state}/${category}` }
    ]
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <section className="py-10 px-4 border-b border-white/5">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-2 mb-4 text-xs text-gray-600">
            ...
            <span>/</span>
            <Link aria-label="Navigation Link" href={`/directory/${country}`} className="hover:text-amber-400">United States</Link>
            <span>/</span>
            <Link aria-label="Navigation Link" href={`/directory/${country}/${state}`} className="hover:text-amber-400">{stateName}</Link>
            <span>/</span>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500">
                {(total ?? 0).toLocaleString()} {categoryName.toLowerCase()} in {stateName}
              </p>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        {/* Category Grid - hide if we already selected one, but we can keep it as alternate categories */}
        {!q && (
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <form method="GET" className="flex gap-2">
            <input
              ...
              placeholder={`Search ${stateName} ${categoryName}...`}
        ...
        {operators && operators.length > 0 ? (
          ...
          <div className="text-center py-16">
            <p className="text-gray-500 mb-4">
              {q ? `No operators found for "${q}" in ${stateName}` : `No ${categoryName} found in ${stateName} yet.`}
            </p>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-10">
            {page > 1 && (
              <Link aria-label="Navigation Link"
                href={`/directory/us/${state}/${category}?page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ''}&sort=${sortBy}`}
