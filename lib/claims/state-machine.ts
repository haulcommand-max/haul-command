// ════════════════════════════════════════════════════════════════
// lib/claims/state-machine.ts
// Claim State Machine — migrated from lib/claim-engine.ts (root)
// CANONICAL location. Root file re-exports from '@/lib/claims'.
// ════════════════════════════════════════════════════════════════
...
export const CLAIM_STATES = [
    ...
    'ownership_granted',
    'profile_started',
    'profile_25',    // ← NEW: triggers "you're 25% done" nudge
    'profile_50',
    'profile_70',
...
export function prequalify(input: PrequalInput): { path: 'fast' | 'guided' | 'review_heavy'; riskBucket: RiskBucket } {
    ...
    if (riskScore > 0.6) return { path: 'review_heavy', riskBucket: 'high' };
    if (riskScore > 0.3) return { path: 'guided', riskBucket: 'medium' };
...
export const NUDGE_SCHEDULES: NudgeSchedule[] = [
    ...
    // ── NEW: Post-earn nudges ──
    {
        situation: 'first_earn_opportunity_spotted',
        delays: ['1h'],
        channel: ['push', 'email'],
    },
    {
        situation: 'corridor_traffic_spike',
        delays: ['0d'],
        channel: ['push'],
    },
];
