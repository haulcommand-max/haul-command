...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'ltree') THEN
    BEGIN EXECUTE 'ALTER EXTENSION ltree SET SCHEMA extensions'; EXCEPTION WHEN OTHERS THEN NULL; END;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_net') THEN
    BEGIN EXECUTE 'ALTER EXTENSION pg_net SET SCHEMA extensions'; EXCEPTION WHEN OTHERS THEN NULL; END;
  END IF;
END $$;
