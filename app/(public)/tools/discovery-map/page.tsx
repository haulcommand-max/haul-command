...
const FLAG: Record<string, string> = { US: '🇺🇸', CA: '🇨🇦', AU: '🇦🇺', GB: '🇬🇧', NZ: '🇳🇿', ZA: '🇿🇦', DE: '🇩🇪', NL: '🇳🇱', AE: '🇦🇪', BR: '🇧🇷', BE: '🇧🇪', SA: '🇸🇦', SG: '🇸🇬', TH: '🇹🇭', IN: '🇮🇳', KR: '🇰🇷' };
                        ...
                        {filtered.map(zone => (
                            <div key={zone.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-6 hover:border-[var(--color-accent)]/30 transition-all">
                                <div className="flex items-center justify-between flex-wrap gap-3">
                                    <div className="flex items-center gap-3 sm:gap-4"><span className="text-2xl sm:text-3xl">{ZONE_ICONS[zone.zone_type] || '📍'}</span><div><h3 className="text-white font-black text-base sm:text-lg">{zone.zone_name}</h3><p className="text-gray-500 text-xs">{FLAG[zone.country_code] || ''} {zone.city} • {zone.zone_type.replace(/_/g, ' ')} • {zone.search_radius_miles}mi</p></div></div>
