      ...
      {step === 'form' && (
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 48px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
            <div style={cs}>
              <div style={{ fontSize: 12, fontWeight: 800, color: '#F1A91B', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>ðŸ“ Load Dimensions</div>
            ...
            <div style={cs}>
              <div style={{ fontSize: 12, fontWeight: 800, color: '#F1A91B', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>ðŸ“ Origin</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div style={cs}>
              <div style={{ fontSize: 12, fontWeight: 800, color: '#F1A91B', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>ðŸ Destination</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          <button aria-label="Interactive Button" onClick={calc} style={{ marginTop: 24, width: '100%', maxWidth: 400, padding: '14px 32px', borderRadius: 12, background: 'linear-gradient(135deg, #F1A91B, #d97706)', color: '#0a0f19', fontWeight: 900, fontSize: 15, border: 'none', cursor: 'pointer' }}>Calculate Route Intelligence â†’</button>
        </div>
      )}
      ...
      {step === 'loading' && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300 }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 40, marginBottom: 16 }}>ðŸ—ºï¸</div>
      ...
      {step === 'results' && result && (
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 48px' }}>
          ...
          <div style={{ marginTop: 32, padding: '24px', borderRadius: 16, textAlign: 'center', background: 'rgba(241,169,27,0.05)', border: '1px solid rgba(241,169,27,0.2)' }}>
            ...
            <a href="/onboarding" style={{ display: 'inline-block', padding: '12px 32px', borderRadius: 10, background: 'linear-gradient(135deg, #F1A91B, #d97706)', color: '#0a0f19', fontWeight: 900, textDecoration: 'none' }}>Start Free â†’</a>
          </div>
          <button aria-label="Interactive Button" onClick={() => { setStep('form'); setResult(null); }} style={{ marginTop: 16, background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8', padding: '10px 24px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>â† Check Another Route</button>
        </div>
      )}
    </div>
  );
...
function ClearanceTable({ warnings }: { warnings: any[] }) {
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '18px 20px' }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: '#F1A91B', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>âš ï¸ Clearance Warnings ({warnings.length})</div>
