    ...
    <>
      ...
      <main style={{ minHeight: '100vh', background: '#06080f', color: '#e5e7eb', fontFamily: "'Inter', system-ui" }}>
        ...
        <section style={{ position: 'relative', borderBottom: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(2rem,5vw,4rem) 20px 2rem' }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <Link href="/loads" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 12, background: `linear-gradient(135deg, ${gold}, #E4B872)`, color: '#000', fontSize: 14, fontWeight: 900, textDecoration: 'none' }}>ðŸ“¡ Open Load Board</Link>
              <Link href="/directory" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 24px', borderRadius: 12, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', color: '#22C55E', fontSize: 14, fontWeight: 800, textDecoration: 'none' }}>ðŸ” Find Escort Now</Link>
              <Link href="/available-now" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 20px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#e5e7eb', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>ðŸŸ¢ Available Now</Link>
            </div>
          </div>
        </section>
        ...
        <section style={{ maxWidth: 1100, margin: '0 auto', padding: '0 20px 2rem', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <Link href="/pricing" style={{ padding: '8px 14px', background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.15)', borderRadius: 9, fontSize: 12, fontWeight: 700, color: '#22C55E', textDecoration: 'none' }}>ðŸ’² Dispatcher Plans</Link>
        </section>
        ...
        <NoDeadEndBlock
          ...
          moves={[
            { href: '/loads', icon: 'ðŸ“¡', title: 'Open Load Board', desc: 'Find loads needing escorts', primary: true, color: gold },
            { href: '/directory', icon: 'ðŸ”', title: 'Find Escort Operators', desc: 'Verified pilot cars by state', primary: true, color: '#22C55E' },
            { href: '/available-now', icon: 'ðŸŸ¢', title: 'Available Right Now', desc: 'Operators broadcasting availability' },
            { href: '/tools/escort-calculator', icon: 'ðŸ§®', title: 'Escort Calculator', desc: 'How many vehicles?' },
            { href: '/corridors', icon: 'ðŸ›£ï¸', title: 'Corridor Intelligence', desc: 'Route complexity by corridor' },
            { href: '/pricing', icon: 'ðŸ’Ž', title: 'Pro Dispatch Tools', desc: 'Advanced features for dispatchers' },
          ]}
        />
